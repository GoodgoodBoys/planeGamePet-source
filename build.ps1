...
if ($BuildPublic) {
    ...
    $publicInstructions = @(
        ...
        '7. 鼠标移到桌宠上，点击“开一局”直接邀请；右侧四个表情可直接点击发送。鼠标移出后整排按钮隐藏，再移入时全部显示。闲置时自动射击，方向随机偏转左右各 15°，左键不控制射击。双方在线时按原先整体向右的路线追逐。',
    ...
    $publicDualInstructions = @(
        ...
        '6. 鼠标移到桌宠上，点击“开一局”直接邀请；右侧四个表情可直接点击发送。鼠标移出后整排按钮隐藏，再移入时全部显示。闲置时自动射击，方向随机偏转左右各 15°，左键不控制射击。双方在线时可测试双向表情。',
...
$testerInstructions = @(
    ...
    '4. 匹配成功后，鼠标移到桌宠上点“开一局”，或右键选“邀请对战”；在另一端点“接受”。右侧四个表情可直接点击发送；鼠标移出后整排按钮隐藏，再移入时全部显示。闲置时自动射击，方向随机偏转左右各 15°，左键不控制射击。',
