...
async def run(folder):
    ...
    os.environ['PLANE_PET_MAX_CONNECTIONS'] = '128'
    spec = importlib.util.spec_from_file_location('capacity_gateway', ROOT / 'gateway/plane_pet_gateway.py')
    g = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(g)
    logging.getLogger().setLevel(logging.WARNING)
    g.TOKENS = {g.token_digest(f'Bearer {index:064x}'): ('1', 'synthetic') for index in range(1, 130)}
    ...
    server = await asyncio.start_server(g.handle_client, '127.0.0.1', 0, backlog=256)
    ...
    try:
        connections.extend(await asyncio.gather(*(open_client(index) for index in range(1, 129))))
        assert g.active_connections == 128
        await open_client(129, False)
    ...
    samples.sort()
    result = dict(connections=128, extra_connection_rejected=True, reconnect_waves=3,
...
if __name__ == '__main__':
    with tempfile.TemporaryDirectory(prefix='plane-pet-capacity-') as folder:
        asyncio.run(asyncio.wait_for(run(Path(folder)), 45))
