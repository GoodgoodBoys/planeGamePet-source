...
STAGE = Path('/opt/plane-pet/staging/battle-3.0.0-20260907')
TARGET = Path('/opt/plane-pet/bin/plane-pet-server')
CANDIDATE = STAGE/'plane-pet-server'
BASELINE = '726d6828dd166b84e96e03e10ca4255a9c977c7ed2a6ece98be28a761adc2a2e'
APPROVED = 'ad2f901ccc9cb4ac93cfbed7a695eb4d3c0608a7fc5c28749b6b7a2a0a17c439'
...
def install(source):
    ...
    fd, name = tempfile.mkstemp(prefix='plane-pet-server.battle-', dir=TARGET.parent)
    os.close(fd)
    temporary = Path(name)
    try:
...
def main():
    ...
    backup = Path(tempfile.mkdtemp(prefix='battle-3.0.0-', dir='/opt/plane-pet/backups'))
    ...
    try:
        ...
        result = {'version': '3.0.0', 'server_sha256': digest(TARGET), 'backup': str(backup),
        ...
        changed = False
        print('BATTLE_SERVER_ACTIVATED ' + json.dumps(result))
    finally:
        if changed:
